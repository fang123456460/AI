...
async function callVisionAPI({ baseUrl, apiKey, model, messages, useJsonMode }) {
  const payload = {
    model,
    messages,
    temperature: 0.25,
...
export async function handler(event) {
  ...
  if (!baseUrl || !apiKey || !model) {
    return jsonResponse(500, {
      error: "AI接口还没有配置好。请在 Netlify 环境变量里设置 AI_BASE_URL、AI_API_KEY、AI_MODEL。",
      setupRequired: true,
    });
  }
  ...
  try {
    ...
    return jsonResponse(400, { error: "请求格式错误，不是有效 JSON。" });
  }

  const { profile, photos } = body;
  if (!profile || !Array.isArray(photos) || photos.length < 1) {
    return jsonResponse(400, { error: "缺少用户信息或照片。" });
  }
  ...
  if (validPhotos.length < 1) {
    return jsonResponse(400, { error: "没有收到有效图片。" });
  }
...
`;
  ...
  try {
    const data = await callVisionAPI({ baseUrl, apiKey, model, messages, useJsonMode: false });
    const text = data?.choices?.[0]?.message?.content || "";
    const report = safeParseReport(text);
    return jsonResponse(200, { report, usage: data.usage || null });
  } catch (error) {
    return jsonResponse(500, {
      error: "AI报告生成失败。优先检查：模型名/API Key/图片过大/函数超时。",
